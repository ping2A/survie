...
pub fn spawn_scheduler_system(
    ...
    if active_stage.phase_time <= 0.0  {
        ...
        active_stage.phase_time = active_stage.stage.spawn_phases[active_stage.current_spawn_phase].duration;
        active_stage.spawn_interval_time = 0.0;

        info!("New stage {:?}", active_stage.current_spawn_phase);
    }
    ...
    info!("[{:?}] New wave {:?} {:?}", active_stage.current_spawn_phase, active_stage, active_stage.stage.spawn_phases[active_stage.current_spawn_phase].spawn_interval);
    ...
    let minimum_enemies_count = active_stage.stage.spawn_phases[active_stage.current_spawn_phase].minimum_enemy_amount;

    match active_stage.stage.spawn_phases[active_stage.current_spawn_phase].pattern {
        SpawnPattern::Random { .. } => {
            random_spawn_pattern(spawner_config, active_stage, spawn_task_receiver,  player_query, minimum_enemies_count, enemies_missing)
        }
        SpawnPattern::Circle { .. } => {
...
fn random_spawn_pattern(
    ...
    player_query: Query<(Entity, &Transform), With<Player>>,
    minimum_enemy_amount: usize,
    enemies_missing: usize,
) {
    let mut enemies_to_spawn = rand::thread_rng().gen_range(0..min(minimum_enemy_amount, enemies_missing));

    info!("Spawn new Random wave {:?}", enemies_to_spawn);
...
fn grouped_spawn_pattern(
    ...
    info!("Spawn new Grouped wave {:?}", enemy_amount);
