...
pub fn player_hit_system(
    ...
    for event in player_enemy_collision_events.read() {
        ...
        if !enemy_damaged_entities.contains(&damaged_entity) {
            ...
            // Our player got some damage, add it to the DamageEvent
            damaged_event.send(DamagedEvent::new(enemy_entity, player_entity));

            // Damned player is dead
