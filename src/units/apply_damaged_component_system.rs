...
pub fn apply_damage_component_system(
    ...
    mut damaged_event: EventReader<DamagedEvent>,
) {
    for event in damaged_event.read() {
        commands.entity(event.target_entity).insert(DamagedEffect::new());
        sound_manager.queue_sound(SoundHandleChannel::Sound(sound_handles.hit_sound.clone()));
    }
}